...
RAPE_STRINGS = [
"`Dear {victim} സ്നേഹിക്കാൻ ഒരുപാട് കാരണങ്ങൾ ഉണ്ടായേക്കും.. എന്നാൽ നിന്നിൽ കാരണങ്ങളല്ല മറിച്ചു ഞാൻ നിന്നിലേക്ക് പകർത്തു നൽകിയ എന്റെ സ്നേഹോപകാരമാണ് എന്റെ പ്രണയം എന്ന് [{user1}](tg://user?id={SURID})`",
]
THANOS_STRINGS = [
...
@bot.on(admin_cmd(outgoing=True, pattern="ayinu$"))
@bot.on(sudo_cmd(pattern="ayinu$", allow_sudo=True))
...
@bot.on(admin_cmd(outgoing=True, pattern="mulla$"))
@bot.on(sudo_cmd(pattern="mulla$", allow_sudo=True))
...
@bot.on(admin_cmd(outgoing=True, pattern="alone$"))
@bot.on(sudo_cmd(pattern="alone$", allow_sudo=True))
